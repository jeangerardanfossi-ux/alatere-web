/**
 * Icônes Lucide partagées (markup inline) + placeholder d'image.
 * Utilisé par toutes les pages portées depuis le handoff « Grommet ».
 */
// Icônes Lucide (markup inline) - généré depuis lucide-static.
// Évite la dépendance CDN du prototype Claude Design.
...
/**
 * Icône rendue comme `<i class="g-ico …"><svg/></i>`. Le wrapper `<i>` est la
 * cible exacte des sélecteurs du prototype (`i`, `.f-domain__icon`, …), donc les
 * CSS portés s'appliquent sans transformation. Le SVG remplit le `<i>` et hérite
 * de `color` (currentColor) et de `stroke-width`.
 */
export function Icon({ name, className = '' }: { name: string; className?: string }) {
  return (
    <i className={`g-ico ${className}`.trim()} aria-hidden="true">
      <svg
        viewBox="0 0 24 24"
        fill="none"
        stroke="currentColor"
...
/* Glyphes de marque (remplis) - Lucide ayant retiré ses icônes de marque. */
const BRAND_ICONS: Record<string, string> = {
  facebook:
    '<path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>',
  google:
    '<path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.76h3.56c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.56-2.76c-.99.66-2.25 1.05-3.72 1.05-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84A11 11 0 0 0 12 23z"/><path fill="#FBBC05" d="M5.84 14.1a6.6 6.6 0 0 1 0-4.2V7.07H2.18a11 11 0 0 0 0 9.87z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1a11 11 0 0 0-9.82 6.07l3.66 2.83c.87-2.6 3.3-4.52 6.16-4.52z"/>',
  linkedin:
    '<path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.225 0z"/>',
};

/** Icône de réseau social (glyphe rempli). */
...
/**
 * Emplacement d'image. Si `src` est fourni, affiche la photo (object-fit cover) ;
 * sinon, montre l'état vide (cadre pointillé + légende) du prototype.
 */
export function ImageSlot({
  ...
  className?: string;
  src?: string;
  /** Cadrage de la photo, ex. "50% 15%" pour montrer le haut (têtes). */
  objectPosition?: string;
}) {
  const cls = [
    'g-img-slot',
    variant ? `g-img-slot--${variant}` : '',
    src ? 'g-img-slot--filled' : '',
    className,
  ]
    .filter(Boolean)
    .join(' ');
  return (
    <div className={cls} role="img" aria-label={cap}>
      {src ? (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={src} alt={cap} loading="lazy" style={objectPosition ? { objectPosition } : undefined} />
      ) : (
        <>
          <Icon name="image" />
          <span className="g-img-slot__cap">{cap}</span>
        </>
      )}
    </div>
  );
